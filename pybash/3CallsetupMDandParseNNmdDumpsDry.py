...
os.system(f'source ~/scratch-shared/npl_miniconda/etc/profile.d/conda.sh')
os.system(f'conda activate deepmd_gpu_poly')
os.chdir('NNmd')

for nCycle in range(totalLammpsCycles):
    # HACK comment below to bypass MD sims
    print('nCycle=',nCycle)
    #os.system('bash ../../pybash/3setupMD.sh')
    ...
    for sysNum, sysFolder in enumerate(systems):
        ...
        os.system('pwd')
        os.system(f'python ../../../pybash/lammpsToJDFTx.py output.dump md {folderIter} {nCycle+LOlammpsNum} {sysNum} md.out {model_devi_f_trust_lo} {model_devi_f_trust_hi} {sampleFreq} > SnapsForAIMD{folderIter}_{nCycle+LOlammpsNum}_{sysNum}')
        ...
        #os.system(f'cp hist.pdf hist{folderIter}_{nCycle+LOlammpsNum}_{sysNum}.pdf')
        #os.system(f'cp MaxOverTime.pdf MaxOverTime{folderIter}_{nCycle+LOlammpsNum}_{sysNum}.pdf')
        os.chdir('..')
