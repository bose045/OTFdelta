...
os.system(f'conda activate deepmd_gpu')
...
launchdir = '/gpfs/u/home/XXXX/XXXXxxxx/scratch-shared/PolyGroup/maceOTFdelta/Autolearn'
scriptdir = os.path.join(launchdir, 'pybash')
...
# if True:
    ...
    try:
        ...
        initDataFile = os.path.basename(glob.glob(r'*_init.data')[0])
        print(f"File found: {initDataFile}")
    except:
        initDataFile = None
    ...
    for iLine, line in enumerate(fpIn):
        ...
        if stepActive and line.startswith("# Energy components:"):
            ...
            os.system("lmp < %s/lammps.in -v struc %s -v base %s"% (scriptdir,str(activeNameFile), str(activeName)))
